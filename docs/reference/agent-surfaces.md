...
| `licell_cmd_doctor` | `licell doctor` | 诊断本机登录态、云端身份/权限/目标资源/域名入口、当前目录项目配置，以及 FC API 的 deploy precheck。 Structured JSON result: 返回本机诊断汇总、计数和逐项检查结果；每个检查项都附带结构化修复建议与后续命令。 Key fields: stage, healthy, checkCount, okCount, warnCount, errorCount, skipCount, context, checks, checks[].id, checks[].status, checks[].summary, checks[].details[], checks[].remediation[], checks[].remediation[].type, checks[].remediation[].text, checks[].remediation[].commandTemplate, checks[].remediation[].commandKey, checks[].remediation[].intent, checks[].nextCommands[], checks[].nextCommands[].commandTemplate, checks[].nextCommands[].commandKey, checks[].nextCommands[].intent, checks[].nextCommands[].priority. Decision guide: Inspect → licell doctor --output json. Auto-generated from the shared licell CLI registry. | `runtime`, `entry`, `dockerDaemon`, `offline` |
