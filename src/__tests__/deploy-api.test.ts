...
describe('executeApiDeploy', () => {
  ...
  it('revalidates an existing network in the effective region before FC deploy', async () => {
    ...
    expect(mockDeployFC).toHaveBeenCalledWith(
      ...
      'index.ts',
      'nodejs22',
      expect.objectContaining({ network: effectiveNetwork })
    );
  });
