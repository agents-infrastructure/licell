...
export function registerEnvCommands(cli: CAC) {
  cli.command('env list', '查看云端环境变量')
    ...
    .action(async (options: { target?: string; showValues?: boolean }) => {
      ...
      const s = spinner();
      s.start(qualifier ? `正在拉取 alias=${qualifier} 的环境变量...` : '正在拉取云端环境变量...');
      try {
        const envs = await pullFunctionEnvs(project.appName, qualifier);
        ...
        s.stop(pc.red('❌ 获取环境变量失败'));
        console.error(formatErrorMessage(err));
        process.exitCode = 1;
      }
    });

  cli.command('env set <key> <value>', '设置云端环境变量（并同步本地 .ali/project.json）')
    .action(async (key: string, value: string) => {
      ...
      const s = spinner();
      s.start(`正在写入环境变量 ${envKey}...`);
      try {
        const envs = await setFunctionEnv(project.appName, envKey, envValue);
        Config.setProject({ envs }, { replaceEnvs: true });
        s.stop(pc.green('✅ 环境变量已写入云端并同步到本地配置'));
        outro('Done.');
      } catch (err: unknown) {
        s.stop(pc.red('❌ 环境变量写入失败'));
        console.error(formatErrorMessage(err));
        process.exitCode = 1;
      }
    });

  cli.command('env rm <key>', '删除云端环境变量（并同步本地 .ali/project.json）')
    .action(async (key: string) => {
      ...
      const s = spinner();
      s.start(`正在删除环境变量 ${envKey}...`);
      try {
        const envs = await removeFunctionEnv(project.appName, envKey);
        Config.setProject({ envs }, { replaceEnvs: true });
        s.stop(pc.green('✅ 环境变量已从云端移除（若存在）并同步本地配置'));
        outro('Done.');
      } catch (err: unknown) {
        s.stop(pc.red('❌ 环境变量删除失败'));
        console.error(formatErrorMessage(err));
        process.exitCode = 1;
      }
    });
